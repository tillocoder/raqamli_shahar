...
class AuthLoginService {
  ...
  static Future<void> getToken(Map<String, String> data, BuildContext context) async {
    try {
      var response = await dio.post('${Urls.baseUrl}${Urls.apiToken}', data: data);
      if (response.statusCode == 200 || response.statusCode == 201) {
        var responseData = response.data as Map<String, dynamic>;
        String? accessToken = responseData["access"] as String?;
        String? refreshToken = responseData["access"] as String?;
        if (accessToken != null) {
      ...
      // ignore: use_build_context_synchronously
      SnackBarHelper.showInternetConnectionError(context, 'Kirish uchun ruxsat yo\'q');
      debugPrint('token ololmadi tasdiqlanmagan user');
      return;
    }
  }

...
class ActivitiesGetListServices {
  ...
  static Future<void> getactivitiesList(int id) async {
    try {
      var response = await dio.get('http://mahalla.ijro-app.uz/api/v1/citizen/activities/347154/');
      if (response.statusCode == 200) {
        debugPrint('STATUS CODE: ${response.statusCode}');

        var responseData = response.data as Map<String, dynamic>;
        var citizenData = responseData['citizen'] as List<dynamic>;
        activities = citizenData.map((e) => Citizen.fromJson(e)).toList(); // Fixed this line
